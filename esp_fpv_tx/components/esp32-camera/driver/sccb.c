...
uint8_t SCCB_Probe() {
  ...
  while (slave_addr < 0x7f) {
    ...
    i2c_master_stop(cmd);
    esp_err_t ret =
        i2c_master_cmd_begin(SCCB_I2C_PORT, cmd, 1000 / portTICK_RATE_MS);
...
uint8_t SCCB_Read(uint8_t slv_addr, uint8_t reg) {
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(TAG,
             "SCCB_Read Failed addr:0x%02x, reg:0x%02x, data:0x%02x, ret:%d",
...
uint8_t SCCB_Write(uint8_t slv_addr, uint8_t reg, uint8_t data) {
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(TAG,
             "SCCB_Write Failed addr:0x%02x, reg:0x%02x, data:0x%02x, ret:%d",
             slv_addr, reg, data, ret);
  }
