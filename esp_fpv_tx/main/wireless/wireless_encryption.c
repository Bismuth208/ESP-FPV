...
wifi_enryption_pair_keys(void)
{
	...
	} while(length < sizeof(pairing_data_t) - 1);

	uart_read_bytes(UART_SYNC_NUM, &xSync, sizeof(pairing_data_t), pdMS_TO_TICKS(500));
	...
	// Wait Receiver for pin re-initialization
	vTaskDelay(pdMS_TO_TICKS(100));

	// Fill self MAC. It will be used by the Receiver
	vWirelessGetOwnMAC(&xSync.ucOtherNodeMac[0]);

	// Send data to the Receiver
	uart_write_bytes(UART_SYNC_NUM, &xSync.ucOtherNodeMac[0], ESP_NOW_ETH_ALEN);
	ESP_ERROR_CHECK(uart_wait_tx_done(UART_SYNC_NUM, pdMS_TO_TICKS(100)));
